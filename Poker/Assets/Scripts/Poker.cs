...
public class Poker : MonoBehaviour {
    public Text PlayerOneHand;
    public Text PlayerTwoHand;
    public int GameNumber = 0;
    public Text GamenumberText;

    //Player One Stats
    string[] PlayerOneHandArray;
    int[] HandOneTypeIndex = new int[5];
    int[] HandOneNumber = new int[5];

    // 0 = HighestCard // 1 = One Pair // 2 = Two Pair // 3 = Three of a Kind //
    int HighestSetOne = 0;
    int HighestSetNumberOne = 0;


    
    //Player Two Stats
    string[] PlayerTwoHandArray;
    int[] HandTwoTypeIndex = new int[5];
    int[] HandTwoNumber = new int[5];

    // 0 = HighestCard // 1 = One Pair // 2 = Two Pair // 3 = Three of a Kind //
    int HighestSetTwo = 0;
    int HighestSetNumberTwo = 0;
    ...
    public void PlayAll() {
        ...
        foreach (string GameCards in Game) {
            if(GameCards != null)
            PlayGame();
        }
    ...
    public void PlayGame() {
        HighestSetOne = 0;
        HighestSetTwo = 0;
        HighestSetNumberOne = 0;
        HighestSetNumberTwo = 0;
        
        if(Game[GameNumber] != null)
        SplitText = Game[GameNumber].Split('|');

        PlayerOneHand.text = SplitText[0];
        PlayerOneHandArray = PlayerOneHand.text.Split(' ');
        PlayerTwoHand.text = SplitText[1];
        PlayerTwoHandArray = PlayerTwoHand.text.Split(' ');

        GameNumber++;
        GamenumberText.text = GameNumber.ToString();
        AssignNumbers(PlayerOneHandArray, HandOneTypeIndex, HandOneNumber);
        AssignNumbers(PlayerTwoHandArray, HandTwoTypeIndex, HandTwoNumber);
        AssignScore(HandOneNumber, HandOneTypeIndex,ref HighestSetOne,ref HighestSetNumberOne);
        AssignScore(HandTwoNumber, HandTwoTypeIndex,ref HighestSetTwo,ref HighestSetNumberTwo);
        if (HighestSetOne > HighestSetTwo)
        {
            Winner = 1;
        }
        else if (HighestSetOne < HighestSetTwo)
        {
            Winner = 2;
        }
        else if (HighestSetOne == HighestSetTwo)
        {
            if (HighestSetNumberOne > HighestSetNumberTwo)
            {
                Winner = 1;
            }
            else if (HighestSetNumberOne < HighestSetNumberTwo)
            {
                Winner = 2;
            }
            else if (HighestSetNumberOne == HighestSetNumberTwo)
        ...
        if (Winner == 1)
        {
            ...
            PlayerOneWins++;
            WriteToDisk.AddOutput(GameNumber-1, WinState.PLAYER1);
        }
        else if (Winner == 2)
        {
            print("Player Two WINS!!!");
            PlayerTwoWins++;
            WriteToDisk.AddOutput(GameNumber-1, WinState.PLAYER2);
        }
        else if (Winner == 0) {
            print("Draw");
            Draws++;
            WriteToDisk.AddOutput(GameNumber-1, WinState.TIE);
        }
    }
    public void AssignNumbers(string[] CardList, int[] TypeIndex, int[] Number)
    {
        int l = 0;
        foreach (string Cards in CardList)
        {
            ...
            int i;
            if (int.TryParse(Cards[1].ToString(), out i))
            {
                Number[l] = i;
            }
            else if (Cards[1] == 'T')
    ...
    public void AssignScore(int[] HandNumber, int[] HandTypeIndex, ref int HighestSet, ref int HighestCard) {
        int i = 0;
        int PairIndex = 0;
        int HighestNumber = 0;
        int HighestPairNumber = 0;
        foreach (int Cards in HandNumber) {
            int l = 0;
            foreach (int CardsT in HandNumber) {
                if (CardsT == Cards && i != l) {
                    PairIndex++;
                    if (CardsT > HighestPairNumber) {
                        HighestPairNumber = CardsT;
                    }
                }
                l++;                
            }
            if (Cards > HighestNumber)
            {
                HighestNumber = Cards;
            }
            i++;
        }
        print("Pairnumber" + PairIndex);
        print("HighestSingleCard " + HighestNumber);
        print("HighestPairCard " + HighestPairNumber);

        if (PairIndex == 2)
        {
            HighestSet = 1;
            HighestCard = HighestPairNumber;
        }
        else if (PairIndex >= 6)
        {
            HighestSet = 3;
